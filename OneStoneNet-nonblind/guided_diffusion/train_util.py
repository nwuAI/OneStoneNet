...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        self.dis_opt = Adam(params=self.mpdis_trainer.master_params, lr=self.lr * 0.01, betas=(0.0, 0.9))
    ...
    def save(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                ...
                # edge unet model parameter save path
                print("edge unet model parameter save path：",bf.join("", filename))
        ...
        if dist.get_rank() == 0:
            # edge unet opt parameter save path
            with bf.BlobFile(
                bf.join("/mnt/data/wxx/guided_gan11/training2", f"edge_unet_opt{(self.step+self.resume_step):06d}.pt"),
                "wb",
            ) as f:
        ...
        if dist.get_rank() == 0:
            # edge_discri_opt parameter save path
            with bf.BlobFile(
        ...
        def save_checkpoint2(rate, params):
            ...
            if dist.get_rank() == 0:
                # image unet parameter save path
                ...
                print("image unet model parameter save path：",bf.join("", filename))
                with bf.BlobFile(bf.join("", filename), "wb") as f:
                    th.save(state_dict, f)
        ...
        if dist.get_rank() == 0:
            # image unet opt parameter save path
            with bf.BlobFile(
        ...
        def save_checkpoint3(rate, params):
            ...
            if dist.get_rank() == 0:
                # # image discriminator model/ema parameter save path
                ...
                print("image discriminator model parameter save path:",bf.join("", filename))
                with bf.BlobFile(bf.join("", filename), "wb") as f:
                    th.save(state_dict, f)

        save_checkpoint3(0, self.mpdis_trainer1.master_params)
        if dist.get_rank() == 0:
            # image discriminator opt parameter save path
            with bf.BlobFile(
